package com.yclenga.utills.providerdownload.download;
...
import com.yclenga.utills.providerdownload.download.provider.DownloadInfo;
...
public class DownloadManager {
    ...
    public long enqueue(Request request) {
        Uri downloadUri=request.mUri;
        long id=0;
        ContentValues values = request.toContentValues(mPackageName);
//        Cursor cursor=new CursorTranslator(queryByUrl(downloadUri), mBaseUri);
        downloadUri = mResolver.insert(Downloads.CONTENT_URI, values);
        id=Long.parseLong(downloadUri.getLastPathSegment());
//        如果数据有提示已经添加到下载列表，是否重新下载
//        if (cursor.getCount()==0){
//            downloadUri = mResolver.insert(Downloads.CONTENT_URI, values);
//            id=Long.parseLong(downloadUri.getLastPathSegment());
//        } else {
//            if (cursor.moveToFirst()){
//                id=cursor.getLong(cursor.getColumnIndex(Downloads._ID));
//            }
//            cursor.close();
//        }
        return id;
    ...
    public Cursor queryByUrl(Uri uri){
        return mResolver.query(Downloads.CONTENT_URI, UNDERLYING_COLUMNS,Downloads.COLUMN_URI+"= ?",
